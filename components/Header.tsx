        ...
        <header className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? "bg-white/80 backdrop-blur-md border-b border-gray-100 shadow-sm" : "bg-transparent"}`}>
            <div className="container-custom flex h-20 items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                    <LanguageSwitcher />
                    <Link href="#" className="hidden md:block text-sm font-medium text-primary hover:underline">{t.login}</Link>
                    <Link href="#" className="px-5 py-2.5 bg-gradient-to-r from-primary-dark to-primary-light text-white text-sm font-medium rounded-full hover:brightness-110 transition-colors shadow-lg shadow-primary/20">
                        {t.getStarted}
                    </Link>
                </div>
            </div>
